...
\begin{frame}{Fonctionnement des exécutables}
	...
	\vfill
		\item Scénarios de stimulation de l'environnement
			\begin{itemize}
			\item Pilote le banc HIL qui génère les stimulis pour le calculateur sous test
			\item Enregistre des variables durant les scénarios
			\end{itemize}
	}
	\uncover<5->{
	\vfill
...
\begin{frame}{Parser : utilisation de Antlr}
	...
	\begin{itemize}[<+->]
	  \setbeamercovered{transparent}
		\item Rédaction d'une grammaire \newline
			...
			\normalsize
		\item Parcours de l'arbre et actions associées\newline
					\footnotesize
	...
	\begin{itemize}[<+->]
	  \setbeamercovered{transparent}
		\item Avant la phase d'exécution des tests: 
			\begin{itemize}
...
\begin{frame}{Générateur : utilisation de Freemaker}
	...
	\setbeamercovered{transparent}
		...
		\uncover<2->{
		\item 3 types de fichiers à générer
		\begin{itemize}[<+->]
			\item Précond Stim
			\item Stim Scénario
			\item GreenTTest $\rightarrow$ contient \textit{ExpectedBehavior}, informations du rapport, \ldots
		\end{itemize}
		}
		\uncover<6->{
		\item générer du code qui sera compilé\newline
			$\rightarrow$ Limite les erreurs à l'exécution des tests
			}
	\end{itemize}
\end{frame}
